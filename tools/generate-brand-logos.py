...
def svg_text(slug: str, brand: str) -> str:
    brand_size = 46 if len(brand) <= 8 else 32 if len(brand) <= 12 else 27
    return f'''<svg xmlns="http://www.w3.org/2000/svg" width="460" height="170" viewBox="0 0 460 170" role="img" aria-label="BTP {brand} logo">
...
def draw_png(slug: str, brand: str):
    if Image is None or ImageDraw is None:
        return
    image = Image.new("RGBA", (460, 170), (255, 255, 255, 0))
    ...
    draw.text((132, 68), brand, fill="#000000", font=fit(brand, 46, 318))
    draw.text((28, 136), TAGLINE, fill="#111111", font=fit(TAGLINE, 14, 404))
